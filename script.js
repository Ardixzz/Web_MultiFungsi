...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchImageApi(url, resultsContainerId, spinnerElement, displayFunction, fileName) { /* ... (Tetap sama) ... */
        ...
        try {
            ...
            if (response.ok && response.headers.get('Content-Type')?.includes('image')) {
                const blob = await response.blob(); const imgUrl = URL.createObjectURL(blob); displayFunction(imgUrl, resultsContainerId, fileName); showToast('Gambar berhasil dibuat!', 'success');
            ...
            console.error('Error fetching Image API:', error); let userMessage = error.message.includes('Failed to fetch') ? 'Gagal terhubung ke API. (CORS atau API offline)' : `Gagal: ${error.message}`; displayError(userMessage, resultsContainerId); showToast(userMessage, 'error');
    ...
    function displaySpotifyDownloadResults(data, containerId) {
        ...
        const mp3Url = data.mp3DownloadLink;
        if (!mp3Url) {
            container.innerHTML = `<p style="color: red;">Tautan MP3 tidak ditemukan.</p>`;
            return;
        }
        ...
        <div class="result-item" style="margin-top: 10px; width: 100%;">
            <h5 style="margin-bottom: 5px;">Audio Original:</h5>
            <audio controls src="${mp3Url}" style="width: 100%;"></audio>
        </div>
        <div class="result-item" style="margin-top: 10px;">
            <div class="download-links" style="width: 100%; justify-content: center;">
                <a href="${mp3Url}" target="_blank" download>Unduh MP3</a>
